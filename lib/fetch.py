"""
Copyright 2022, pygame wasm, All rights reserved.

Borrowed from https://github.com/pygame-web/pygbag/ under the MIT license.

Originally pygbag.support.cross.aio.fetch
"""
...
async def preload(
    chroot=None, chdir=True, silent=FS_SILENT, debug=FS_DEBUG, isolated=False
):
    ...
    while len(preload_list):
        ...
        if not filename.is_file():
            ...
            if sys.platform in ("emscripten", "wasi"):
                async with platform.fopen(full_url, "rb") as source:  # type: ignore
...
window.Fetch.GET = function * GET (url)
{
   ...
   .catch(err => {
    ...
    yield content;
            ...
            """
            try:
                platform.window.eval(self._js_code)  # type: ignore
    ...
    def print(*args, default=True):
        try:
            for i in args:
                platform.window.console.log(i)  # type: ignore
        except AttributeError:
            pass
    ...
    async def get(self, url, params=None, doseq=False):
        ...
        if self.is_emscripten:
            ...
            await asyncio.sleep(0)
            content = await platform.jsiter(  # type: ignore
                platform.window.Fetch.GET(url + "?" + query_string)  # type: ignore
            )
            if self.debug:
                self.print(content)
            self.result = content
    ...
    async def post(self, url, data=None):
        ...
        if self.is_emscripten:
            await asyncio.sleep(0)
            content = await platform.jsiter(  # type: ignore
                platform.window.Fetch.POST(url, json.dumps(data))  # type: ignore
            )
            if self.debug:
                self.print(content)
            self.result = content
